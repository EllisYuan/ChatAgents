...
Vite 会把 `/api` 与 `/health` 代理到本地 FastAPI（默认 `http://127.0.0.1:8000`）。**8000 是后端在四处配置里的统一端口**，后端用 `scripts/dev.py serve` 起就已经钉在这个端口上；两边不一致时代理会返回自造的 500 空响应，前端看到的是「获取会话列表失败：500」而不是后端的真实错误。
